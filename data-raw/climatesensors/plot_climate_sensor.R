...
working_directory="/PhD_docs/experiment_1001G/experiment_field_1001g_share/climate_sensors/"
...
### Parse files and plot ### Madrid #####
#
# finalfilelist= subset( sensor_files_merge, sensor_files_merge$Site=="Madrid")
#
# numberfiles =dim(finalfilelist)[1]
# print ("Sensors in Madrid: ")
# numberfiles
#
# working_directory="~/GoogleDrive/PhD/experiment_1001G/experiment_field_1001g_share/climate_sensors/"
# pdf(paste(working_directory,"water_temp_series_MADRID.pdf",sep="/"),height =8 ,width = 15, useDingbats = F)
#
# counter=0
# for (sensor in finalfilelist$name  ) {
#   file=as.character(finalfilelist[sensor,"filename"])
#   name=as.character(finalfilelist[sensor,"name"])
#   posnumber= as.numeric(finalfilelist[sensor,"Position"])
#   data<-read.csv(file)
#   head(data);tail(data)
#
#   if( 0 %in% dim(data) ){
...
#   data$capture_ts<-strptime(data$capture_ts, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
#   class(data$capture_ts)
#   data<-subset(data,data$capture_ts >= as.POSIXct("2015-11-16 00:00:00") )
#
#
# colortemp= as.character(finalfilelist[sensor,"Color_temp"])
# colorwater= as.character(finalfilelist[sensor,"Color_water"])
...
#
# par(mar = c(8, 4, 4, 4) + 0.1)
# plot(y = data[,columnames[2]] ,x=data$capture_ts, ylab=columnfinalnames[2], frame.plot=T,     cex=0,xlab="",axes=F,col="blue",xlim = as.POSIXct(c(range(data$capture_ts) )+ c(hours(0),hours(3))) ,ylim=c(-0, 45), xaxt="n")
#
#   lines(y = data[,columnames[2]] ,x=data$capture_ts,col=colorwater,lty=as.numeric(shapewater))
#   lines(y = data[,columnames[3]] ,x=data$capture_ts,col=colortemp,lty=as.numeric(shapetemp))
#
# # add  axis and grid
#
#     axis(2, ylim=c(0,40),at=seq(0,40,5),col="black",las=1)
#   axis(4, ylim=c(0,40),at=seq(0,40,5),col="black",las=1)
#   mtext("Temp (C)",side=4,line=2.5)
#   labelspretty=pretty(range(data$capture_ts),15)
#
#   abline(h=seq(0,40,5), v=labelspretty, col="gray", lty=3)
#
# # add the sensor name in the water lines
# text(y = tail(data[,columnames[2]], n=1) ,x= as.POSIXct(tail(data$capture_ts, n=1)+hours(3))   ,paste(posnumber,name,sep="-"))
#
...
# # library(plyr)
# # data$week <- format(data$capture_ts, format="%Y-%U")
# # ddply(data, .(week), summarize, air_temperature_celsius=sum(air_temperature_celsius))
# # unique(data$week)
#
#
#
