## DOWNSAMPLING EASY ##
# setwd("/ebio/abt6/mexposito")
setwd("/home/moises/")
setwd("/ebio/abt6/mexposito/")
setwd('/media/moisesexpositoalonso/Data/')
getwd()
...
nalocations<-read.table("INVESTIGACION/PhD/project_1001G_history/id_NAlocations")
nalocations # these are the positions
nalocations<-c(440  ,500  ,502  ,504  ,798 ,1119)
## Coverage intersection congruence

cic<-read.table("INVESTIGACION/PhD/project_1001G_history/id_coverage_intersection_congruence")
cic
covintcro<-data.frame(cic)
colnames(covintcro)="id"

##### who neanderthals  #####
neander<-read.table("INVESTIGACION/PhD/project_1001G_history/who_are_neanderthals_read.txt",header=T,sep=" ")
# neander<-read.table("GoogleDrive/INVESTIGACION/PhD/project_1001G_history/neanderthals/who_are_neanderthals_read.txt",header=T,sep=" ")
...
load("R/2014-11-4_1001g/ibs1001g.R")
kkk[1:5,1:5]
X<-kkk
diag(X)<-rep(NA,1135)
...
# newX<-XX[-unique(col(XX)[which(XX < 0.01)]), -unique(col(XX)[which(XX  < 0.01)])]
...
newgeod<-geod[-unique(row(geod)[which(geod < cutoff)]) ,
              unique(row(geod)[which(geod < cutoff)])]
...
# filefinaldata<-"GoogleDrive/INVESTIGACION/PhD/1001G_downsample_523.csv"
# write.csv(FINALdata,file=filefinaldata)

# ####### merging and writing downsamples  ##########
# GROUPS<-read.table('GoogleDrive/INVESTIGACION/PhD/project_1001G_history/fineSTRUCTURE/genomeID_RECEIVERS')
...
# # write.csv(FINALwithGROUPSwithEXTRA,file="GoogleDrive/INVESTIGACION/PhD/1001G_downsample_&extra_533.csv")
#
