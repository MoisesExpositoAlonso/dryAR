...
, starting in the sowing day and until the observed peak of green pixels per pot. The three parameters $a$, $b$, and $c$, were stored together with several less complex indicators of growth: an analogous linear model, the day that over 1000 green pixels were observed (~ 1cm^2^ as pixels resolution is ~xyz), and a total count of green and red pixels through all timepoints. This data columns comprise `r library(dryAR); data(veg); nrow(veg)` with non-missing data. 
