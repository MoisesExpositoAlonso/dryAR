...
### How frequent flowering was measured

```{r}
data("flowering_madrid")
uniqueflowering<-sort(unique(flowering_madrid$FT.date)  ) - sowingday('madrid') 
difflowering<-diff(uniqueflowering)
summary(fn(difflowering))

data("flowering_madrid")
uniqueflowering<-sort(unique(flowering_tuebingen$FT.date)  ) - sowingday('tuebingen') 
difflowering<-diff(uniqueflowering)
summary(fn(difflowering))


```
Flowering was recorded on average ever 1 day, while in Tubingen every 2 days.
...
p1=p1 + xlab('Flowering time since sowing (days)') + scale_alpha(guide = 'none')
save_plot(filename = 'figs/Figure_flowering_distribution.pdf',base_height = 5,base_width = 5, plot=p1)
p1
```
...
f1=ggplot(f,aes(x=row,y=col , fill=flowering ))+geom_tile() + 
  scale_fill_gradientn("Flowering date",
                       colours= make.pallete.contrast( vecol = rev(brewer.pal(10,name = "PRGn")),contrast=2,raw=T ),
                        limits=c(60, 180)
  ) #+
...
f2=f2+ theme(axis.line=element_blank(),axis.text.x=element_blank(),
          ...
          panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),
          plot.background=element_blank()) +ggtitle("Tübingen")
