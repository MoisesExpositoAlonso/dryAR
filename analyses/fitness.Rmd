...
ggplot(field.s) +
  geom_histogram(aes(x=Fruits,group=water,fill=water,alpha=0.3)) + 
...
ggplot(field.s) +
  geom_histogram(aes(x=Survival_fruit,group=water,fill=water,alpha=0.3)) + 
  scale_fill_manual(values = c("h"="navy","l"="red3"))+
  facet_grid(site~indpop)+
  theme_bw() +
  xlab('Survival prob.') + 
...
ggplot(field.c) +
  ...
  facet_grid(site~indpop)+
  theme_bw() +
  xlab('Lifetime fitness (% survival x seeds)') + 
